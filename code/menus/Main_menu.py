...
class Main_menu(Menu):  # Herança: Main_menu herda da classe base Menu
    def __init__(self, screen):
        ...
        """
        super().__init__(screen)  # Chamada ao construtor da classe base (Menu)
        ...
        self.button_infos = {
            "Confirm": self.options.confirm, 
            "New Game": self.options.new_game, 
    ...
    def display_menu(self, jogo):
        ...
        if selected_option == self.options.confirm:  # Ação ao confirmar o login
            if self.login.verify_login(self.user_text, self.password_text):  # Verifica credenciais
